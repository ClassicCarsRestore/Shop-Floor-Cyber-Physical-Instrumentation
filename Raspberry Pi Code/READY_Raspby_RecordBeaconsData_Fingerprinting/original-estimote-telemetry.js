...
noble.on('discover', function(peripheral) {
  ...
  if (telemetryPacket) { 
    ...
    // build custom packet with needed information from telemetryPacket
    if(telemetryPacket.subFrameType == 'B') {
      var outputPacket = {}
      outputPacket.shortIdentifier = telemetryPacket.shortIdentifier
      outputPacket.batteryLevel = telemetryPacket.batteryLevel
      outputPacket.RSSI = peripheral.rssi
      outputPacket.temperature = telemetryPacket.temperature

      process.stdout.write(JSON.stringify(outputPacket) + '\n');
    }
  }
});